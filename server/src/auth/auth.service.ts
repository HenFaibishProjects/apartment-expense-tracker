...
export class AuthService {
  ...
  async validateUser(email: string, pass: string): Promise<ApartmentUser> {
  ...
  async login(user: ApartmentUser): Promise<{ access_token: string }> {
  ...
  async registerUser(dto: RegisterDto): Promise<ApartmentUser> {
  ...
  async activateUser(token: string): Promise<ApartmentUser | null> {
  ...
  async confirmWithCode(email: string, code: string) {
    const user = await this.usersRepo.findOne({
      where: { email: email, activationCode: code },
    });
